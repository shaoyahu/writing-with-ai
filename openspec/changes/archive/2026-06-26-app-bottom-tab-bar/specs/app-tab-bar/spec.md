...
### Requirement: Bottom tab bar with three slots and a raised center FAB

应用 MUST 在主屏(`AppShell`)底部渲染 Material3 `NavigationBar`,共 3 槽,槽位从左到右依次为:

| 槽位 | 类型 | 选中态图标 | 未选中态图标 | 文案资源 |
| --- | --- | --- | --- | --- |
| 1 | 标准 `NavigationBarItem` | `Icons.Filled.Notes` | `Icons.Outlined.Notes` | `R.string.tab_notes` |
| 2 | 圆形凸起 `FloatingActionButton`(`Modifier.offset(y = -16.dp)` 抬出 bar 上沿、`containerColor = colorScheme.primary`、`shape = CircleShape`、尺寸 ≥ 56.dp) | `Icons.Filled.Add`(`contentDescription = R.string.tab_new_note_cd`) | — |
| 3 | 标准 `NavigationBarItem` | `Icons.Filled.Person` | `Icons.Outlined.Person` | `R.string.tab_my` |

中央槽位 MUST 是 `Box` 占位的"空槽"(内部放 FAB,本身不响应 tab 选中)。

#### Scenario: App 启动渲染 tab bar
- **WHEN** `AppNav` 完成 consent gate 并 navigate 到 `AppShell`
- **THEN** `AppShell` 渲染 `Scaffold(bottomBar = { AppTabBar(...) })`,3 槽可见,中央 FAB 浮起在 bar 上沿

#### Scenario: 中央 FAB 始终抬出 bar
- **WHEN** `AppTabBar` 渲染
- **THEN** 中央 `FloatingActionButton` 视觉上**高于**两侧 `NavigationBarItem`(`Modifier.offset(y = -16.dp)` 或等效),`Modifier.shadow(8.dp)` 阴影存在

#### Scenario: 中央 FAB 不显示文案
- **WHEN** `AppTabBar` 渲染中央槽位
- **THEN** FAB 内只渲染 `Icon(Icons.Filled.Add)` + `contentDescription`(`R.string.tab_new_note_cd`),不显示 label 文本

#### Scenario: 1 / 3 槽有图标 + 文案
- **WHEN** `AppTabBar` 渲染两侧槽位
- **THEN** `NavigationBarItem` 含 `icon = { Icon(...) }` + `label = { Text(stringResource(...)) }`,文案来自 strings.xml
