...
`AppNav.kt` MUST 注册以下额外类型安全(`@Serializable`)目的地(custom-prompt-template 增量):
- `Settings` — SettingsScreen 主屏入口(从 QuickNoteListScreen overflow menu 跳)
- `SettingsPromptTemplate` — PromptTemplateScreen 入口(从 SettingsScreen 跳)

`composable<Settings>` block MUST 调 `feature/settings/SettingsEntry.SettingsRoute(navController, onConsented = ...)`;`composable<SettingsPromptTemplate>` block MUST 调 `feature/settings/SettingsEntry.PromptTemplateRoute(onBack = { navController.popBackStack() })`。两个 route MUST NOT 在 widget Intent 启动路径内(继续走 M4-2 widget Intent 启动到 quicknote/edit 或 quicknote/detail)。
...
#### Scenario: widget Intent 不进 Settings 路径
...
#### Scenario: Settings 入口在 QuickNoteListScreen overflow menu
