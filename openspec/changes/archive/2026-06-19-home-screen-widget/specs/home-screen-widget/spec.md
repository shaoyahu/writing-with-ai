...
把 M1-M3 的"App 内主动记录"扩展为"随时随地随手记"(roadmap §3.3 + §8)。定义 Glance 桌面小组件 2x2 / 4x2 双 size mode:`QuickNoteWidgetReceiver` 桥接 widget host + `QuickNoteWidget` 渲染 + `QuickNoteWidgetUpdater` 主路径刷新 + `QuickNoteWidgetWorker` WorkManager 兜底 15min 周期 + `PendingIntent` / `ActionCallback` 路由到 MainActivity 指定 nav route。
