...
`build.gradle.kts` MUST 设 `buildConfigField("Boolean", "CONSENT_GATE_ENABLED", "true")` 不含 apikey 字段;`gradle.properties` MUST 不含 apikey 值。
...
- **THEN** `<cloud-backup>` + `<device-transfer>` 均含 `<exclude domain="file" path="writingwithai_secure_prefs.xml" />`
