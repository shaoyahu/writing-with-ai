...
#### Scenario: 新建入口
- **WHEN** tab bar 中央 FAB 或列表空状态"新建"按钮被点击(app-bottom-tab-bar 后列表自身无 FAB)
...
- **WHEN** 用户从 tab bar 中央 FAB / 列表空状态"新建"按钮进编辑器(app-bottom-tab-bar 后列表自身无 FAB)
...
### Requirement: QuickNoteListScreen no longer hosts overflow menu or FAB (app-bottom-tab-bar)
...
### Requirement: Export-all ZIP button moves from list to SettingsDataScreen (app-bottom-tab-bar)
