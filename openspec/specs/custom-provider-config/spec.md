# custom-provider-config Specification

## Purpose

自定义 provider 配置功能:用户在表单中填 apikey / baseUrl / model，app 据此组装 ProviderConfig 并通过 AiGateway 调用。Custom Provider 支持 OpenAI 兼容和 Anthropic 兼容两种协议切换。

Synced from OpenSpec change `custom-provider-api-format`(2026-07-03)。
