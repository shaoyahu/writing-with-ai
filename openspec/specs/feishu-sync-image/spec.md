# feishu-sync-image Specification

## Purpose

飞书双向同步的图片附件支持:markdown 同步成功后,串行上传 `note_attachments` 表的图片素材,作为独立 image block 追加到飞书 doc 末尾,文字与图片不交叉。

Synced from OpenSpec change `feishu-sync-image-support`(2026-07-08)。
