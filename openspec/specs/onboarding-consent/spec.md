...
## Purpose
TBD - created by archiving change `onboarding-consent`(2026-06-19)。定义首次启动隐私条款同意门 + 版本号管理 + 同意状态持久化的契约。
...
`OnboardingScreen` MUST 从 `assets/privacy_policy_<lang>.md` 读取条款文本(系统语言为英文时读 `en`,其他读 `zh`),用 Markdown 渲染器显示;底部"同意"按钮在用户**未实际滚动过内容** 时 MUST 处于 `enabled = false` 状态(`firstVisible > 0` + `lastVisible >= total - 1` 双条件,避免短文一键同意);滚动到底部时启用。
...
#### Scenario: 滚动到底部解锁
- **WHEN** 用户滚动 `LazyColumn` 且 `firstVisibleItemIndex > 0` 且 `lastVisibleItemIndex >= totalItemsCount - 1`
- **THEN** "同意"按钮 `enabled = true`;否则 `enabled = false`(无法点击)
...
`ConsentStore` MUST 暴露 `consentFlow: StateFlow<ConsentState>`(combine 三 key + `stateIn(Eagerly, EMPTY)`),`setAccepted(version: Int, at: Long)`,`isConsented(currentVersion: Int): Boolean`(suspend,读 StateFlow first())。DataStore 文件名 MUST 为 `consent_store`(默认 preferences 即可,无需加密,只是布尔 + 数字)。
...
- **THEN** 后续 `consentFlow.value` 返回 `ConsentState(accepted=true, acceptedAt=1700000000000L, version=1)`;进程重启后值不变
...
#### Scenario: 同意门回滚逃生口
- **WHEN** `BuildConfig.CONSENT_GATE_ENABLED = false`,`WritingApp.onCreate` 同步 `runBlocking { consentStore.setAccepted(...) }` 写默认 consent
- **THEN** 后续 `AppNav.LaunchedEffect(Unit)` 看到 `accepted=true` 直接跳过 onboarding 路由(无死锁)
...
#### Scenario: app-shell 也通过 OnboardingEntry 引用(M5 polish)
...
#### Scenario: Compose UI test covers scroll-to-bottom unlock(M5 polish)
- **WHEN** `app/src/test/java/com/yy/writingwithai/feature/onboarding/OnboardingScreenUiTest.kt` 文件存在
- **THEN** 用 `RobolectricTestRunner` + `@Config(sdk=[34])` + `createComposeRule()` 验证 scroll-to-bottom 解锁行为
