# app-dropdown-menu Specification

## Purpose

应用统一下拉菜单组件：`AppActionDropdown`(操作菜单) 和 `AppSelectionDropdown`(选择器)。统一容器样式、圆角、elevation、字号、icon 颜色等 token,方便功能模块复用同一套 UI 规范。

Synced from OpenSpec change `unify-dropdown-menu-style`(2026-07-03)。
